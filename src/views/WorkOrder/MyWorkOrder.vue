              ...
              >
                <template v-slot="scope">
                  <span
                    ...
                    class="changkan"
                    @click="addTc('cancellog', scope.row.no)"
                  >撤回</span>
                  <span
          ...
          <div :class="[
            ...
            <div class="tanchuang_center">
              <div class="tanchuang_center_cen">
                ...
                >
                  <selectEquipment v-on:addTc="addTc" :floor="floor" :floorId="formValidator.areaId"></selectEquipment>
                </div>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        content: [{ required: true, message: '请输入内容', trigger: 'change' }],
        areaId: [{ required: true, message: '请选择地点', trigger: 'change' }],
      },
      list: [],
      total: 0,
      pageIndex: 1,
  ...
  methods: {
    ...
    async WorkOrderDiscard(item) {
      ...
      if(node.data.model.name.indexOf(keyword) != -1){
        node.text= '('+node.data.model.code+')'+node.data.model.name
        return true
      }
    },
    // 转单
    ...
    async addTc(type, item) {
      ...
      if (type == 'add') {
        if (item === '') {
          ...
          this.formValidator.level = 2
          this.formValidator.content = ''
          this.formValidator.subTypeNo = this.subTypeData[0].options[0].value
          this.formValidator.source = 2
          this.DealingUserNo = ''
          ...
          this.floor = item.areaName
          this.formValidator.areaName = item.areaName
