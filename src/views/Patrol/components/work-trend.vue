...
export default {
  ...
  mounted() {
    this.dayList = this.hartTrend
    this.alarmLevelqus() //报警趋势
  },
  watch: {
     'hartTrend': function(newVal, oldVal) {
      console.log(newVal, oldVal);
      //数据一变化更新数值
      this.dayList = newVal //newVal即是chartData
  ...
  methods: {
    alarmLevelqus() {
      var _self = this
      let resdata = _self.dayList
      console.log(resdata);
      var xData = (function() {
        let count = resdata.yData
        let data = []
        for (var i = 0; i < resdata.xData.length; i++) {
          data.push(resdata.xData[i].slice(0, 10))
          // count.push(resdata[i].total)
        }
        return { data, count }
      })()
      var shi = ['10:00', '11:00', '12:00', '13:00', '14:00']
      var option = {
      ...
      alarmLevelqus_chart.on('legendselectchanged', function(obj) {
        ...
        if (name == '时') {
          option.xAxis[0].data = hourData.data
        }else{
