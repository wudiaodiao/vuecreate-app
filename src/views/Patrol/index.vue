 ...
 * @LastEditTime: 2021-11-23 15:14:42
        ...
        <div :class="['shiftClass_left',$store.state.dtab?'index_off':'index_no']">
          <div class="left_tot">
            <div class="left_tot_top">
              <span>
                <img
                  src="../../assets/image/index/partial-menu-top.png"
                  alt
                />
              </span>
              <h3>巡检管理</h3>
            ...
            <ul class="left_tot_list">
              <li
                v-trigger
                :data-sunC="itemkey"
                v-for="(item,itemkey) in $store.state.PatrolTask.children"
                :key="item.model.name"
                :class="[
...
export default {
  ...
  watch: {
    ...
    // console.log(to.path);
    if(to.path=='/patrolDefend/PatrolTask'){
       this.$router.push('/patrolDefend/PatrolTask')
        this.$store.commit('setEnergyClick', '巡检记录')
    }
      if (
        from.path == '/patrolDefend/PatrolItem' &&
        to.path == '/patrolDefend'
      ) {
        this.$router.push('/patrolDefend/PatrolItem')
      }
    },
  },
  unmounted() {},
  ...
  directives: {
    trigger: {
      ...
      inserted(el, binging) {
        ...
        console.log(vars2);
        if (vars2 != undefined) {
           if (el.getAttribute('data-sunC') == 3) {
            el.click()
            this.$store.commit('setEnergyClick', '巡检记录')
            console.log(this.$store.state.EnergyClick);
          }
          // this.$router.push('/patrolDefend/PatrolTask')
          // this.$store.commit('setEnergyClick', '巡检记录')
          return
