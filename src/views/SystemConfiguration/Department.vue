      ...
      <div :class="['EquipmentType','public_page',$store.state.dtab?'index_off':'index_no'] ">
        <div class="content">
          <div
            ...
            v-if="$Tools.accessControl( this.$store.state.userGetUserRole.resList,'EditDepartment')"
          >
            <ul>
              <li
                class="gen"
            ...
            <div class="tanchuang_center">
              <div class="tanchuang_center_cen">
                ...
                <div class="tanchuang_center_cen_bottom">
                  <button
                    @click="nodeShowType=='addRootNode'||nodeShowType=='addChildrenNode'?DepartmentAdd('formValidator'):nodeShowType=='edChildrenNode'?DepartmentUpdate('formValidator'):''"
  ...
  methods: {
    async addNodeDel() {
      ...
      this.$confirm('此操作将删除, 是否继续?', '提示', {
        ...
        .then(async () => {
          const res = await that.$api.MMS.Department.Delete({
          ...
          if (res.ok) {
            ...
            that.DepartmentGetAllList('formValidator') //更新部门
          } else {
            that.$notify({
              title: '警告',
              message: res.message,
    ...
    async DepartmentUpdate(formName) {
      //更新
      this.$refs[formName].validate(async (valid) => {
        if (valid) {
          const res = await this.$api.MMS.Department.Update({
            Id: this.equipmentActiveId,
            Name: this.formValidator.inputNodeName,
          })
          if (res.ok) {
            this.nodeShow = false
            this.DepartmentGetAllList() //更新部门
    ...
    async DepartmentAdd(formName) {
      //设备类别添加
      this.$refs[formName].validate(async (valid) => {
        if (valid) {
          const res = await this.$api.MMS.Department.Add({
            Id: this.equipmentActiveId,
            Name: this.formValidator.inputNodeName,
          })
          if (res.ok) {
