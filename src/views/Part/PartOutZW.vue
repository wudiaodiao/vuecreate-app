                  ...
                  <div class="PartOut_tccenter">
                    <div class="PartOut_tccenter_top">
                      <ul class="left">
                        ...
                        <li>
                            <span class="bianji">领用类型:</span>
                            <i>{{ gettype(approvalItem.type) }}</i>
...
export default {
  ...
  methods: {
    ...
    async PartInOutApprove(flag) {
      //批准出库
      const res = await this.$api.MMS.PartInOutZW.Approve({
    ...
    async getPartInOutGetList() {
      //出库审批列表
      const res = await this.$api.MMS.PartInOutZW.GetList({})
      if (res.ok) {
